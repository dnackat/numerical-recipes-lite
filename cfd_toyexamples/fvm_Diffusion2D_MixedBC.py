...
heat diffusion problem with mixed boundary conditons. The physics is governed 
by the equation:
...
fc_x = np.linspace(0.,domain_length,numcells)  # x face centroids
fc_y = np.linspace(0.,domain_length,numcells)  # y face centroids
fxx, fyy = np.meshgrid(fc_x,fc_y)       # Create a 2D grid with face centroids
cc_x = (fc_x[:-1] + (fc_x[1:]))/2.      # x cell centroids
cc_y = (fc_y[:-1] + (fc_y[1:]))/2.      # y cell centroids
cxx, cyy = np.meshgrid(cc_x,cc_y)       # Create a 2D grid with cell centroids
...
#%%
# Source terms
S = 100.*cc*dx      # Linear source term, S*d(vol) = 6*x_centroid*(dx*1.0)
...
b[0] += (k/(cc[0]-fc[0]))*phi_first    # Add the boundary value for first face
b[-1] += (k/(fc[-1]-cc[-1]))*phi_last  #  Add the boundary value for last face
