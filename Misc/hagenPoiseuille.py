...
plt.rcParams.update({
    ...
    "font.family": "serif",
    "font.sans-serif": ["Helvetica"]})
plt.title("Non-dimensionalized Oil Flow Rate",fontsize=24)
plt.plot(H1_by_H,Q,'blue',label=r'$\frac{\mu_{o}}{\mu_{w}} = 10$',linewidth=2)
...
plt.ylim((0,np.max(Q)))
plt.xticks([i for i in np.arange(0,1.2,0.2)])
plt.yticks([i for i in np.arange(0,np.max(Q)+1,1)])
plt.grid(axis="both")
